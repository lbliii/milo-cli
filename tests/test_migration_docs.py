...
_COMPARISON = _MIGRATE_DIR / "comparison.md"


def _recipe_pages() -> list[Path]:
    return sorted(
        path
        for path in _MIGRATE_DIR.glob("from-*.md")
        if path.name not in {"_index.md", "comparison.md"}
    )
...
def test_comparison_links_official_competitor_docs():
    text = _COMPARISON.read_text(encoding="utf-8")
    expected = [
    ...
    assert not missing, f"Comparison page is missing official source URL(s): {missing}"
