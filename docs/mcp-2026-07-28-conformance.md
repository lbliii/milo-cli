...
| Streamable HTTP headers and errors | `CLI.asgi_app()` validates `Mcp-Method`, `Mcp-Name`, `MCP-Protocol-Version`, Base64 sentinel values, recognized `Mcp-Param-*` headers, Origin, media types, and bounded bodies. Header disagreement returns HTTP 400 / `-32020`; unknown methods return HTTP 404 / `-32601`. | Implemented for modern POST request/response and request-scoped SSE; legacy HTTP sessions/GET streams intentionally unsupported |
| HTTP authorization | Sync/async bearer-token callback, header-only tokens, RFC 9728 protected-resource metadata and challenge URL, and exact Origin allowlist. The callback owns signature, issuer, audience, expiry, and scope policy. | Implemented as opt-in ASGI hooks; standalone non-loopback binds require an explicit unsafe override |
...
- [x] Dependency-free ASGI HTTP parity, header/auth boundary tests, and parallel 3.14t tool-call proof
