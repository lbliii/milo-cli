...
:::{tip}
Listeners fire after every dispatch. For expensive operations (API calls, file writes), trigger them from [[docs/build-apps/sagas|sagas]] instead — sagas run on the thread pool and won't block rendering.
:::
...
For details on commands, see [[docs/build-apps/commands-effects|Commands]]. For sagas, see [[docs/build-apps/sagas|Sagas]].
