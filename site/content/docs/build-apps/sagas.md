...
For one-shot effects (fetch a URL, write a file, dispatch the result), consider using [[docs/build-apps/commands-effects|Commands]] instead. Commands are simpler — a plain function instead of a generator — and handle the dispatch-result pattern automatically.
